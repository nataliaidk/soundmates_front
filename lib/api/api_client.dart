...
class ApiClient {
  ...
  Future<http.Response> register(RegisterDto dto) async {
    ...
    if (headerAuth != null && headerAuth.isNotEmpty) {
      ...
      if (access != null)
        await authNotifier!.setTokens(access: _sanitizeToken(access));
    }
    return resp;
  }
  ...
  Future<http.Response> updateUserWithTags(
    ...
    return await _withRefreshRetry(
      () => http.put(uri, headers: headers, body: jsonEncode(inner)),
    );
  }

  Future<http.Response> updateArtistProfile(
    UpdateArtistProfile dto, [
    List<String>? tags,
  ]) async {
    final uri = _uri('/users/profile');
    final headers = await _authHeaders();
  ...
  Future<http.Response> updateBandProfile(
    UpdateBandProfile dto, [
    List<String>? tags,
  ]) async {
    final uri = _uri('/users/profile');
    final headers = await _authHeaders();
  ...
  Future<void> saveTokensFromResponseBody(String body) async {
    ...
    try {
      ...
      String? findToken(dynamic node, List<String> keys) {
        ...
        if (node is Map) {
          for (final k in keys) {
            if (node.containsKey(k) && node[k] is String)
              return node[k] as String;
          }
      ...
      final access = findToken(decoded, [
        'accessToken',
        'access_token',
        'token',
        'jwt',
      ...
      final refresh = findToken(decoded, [
        'refreshToken',
        'refresh_token',
        'refresh',
      ]);
  ...
  Future<http.Response> unmatch(String userId) async {
    final uri = _uri('/matching/unmatch/$userId');
    final headers = await _authHeaders();
    return await _withRefreshRetry(() => http.delete(uri, headers: headers));
  }

  Future<http.Response> checkMatchExists(String userId) async {
    final uri = _uri('/matching/match/exists/$userId');
    final headers = await _authHeaders();
    return await _withRefreshRetry(() => http.get(uri, headers: headers));
  }
  ...
  Future<http.Response> getPotentialMatchesBands({
    int limit = 20,
    int offset = 0,
  }) async {
  ...
  Future<http.Response> getMessagePreviews({
    int limit = 20,
    int offset = 0,
  }) async {
  ...
  Future<http.Response> getMusicSamples({
    int limit = 20,
    int offset = 0,
  }) async {
  ...
  Future<http.Response> getProfilePictures({
    int limit = 20,
    int offset = 0,
  }) async {
  ...
  Future<http.Response> getProfilePicturesForUser(
    String userId, {
    int limit = 20,
    int offset = 0,
  }) async {
  ...
  Future<UserDto?> getMyProfileDto() async {
    ...
    try {
      ...
      if (decoded is String) {
        ...
        if (parsed is Map)
          return UserDto.fromJson(Map<String, dynamic>.from(parsed));
  ...
  Future<http.Response> updateUser(UpdateUserProfileDto dto) async {
    ...
    return await _withRefreshRetry(
      () => http.put(uri, headers: headers, body: jsonEncode(envelope)),
    );
  }
  ...
  Future<http.Response> deleteUser(PasswordDto dto) async {
    ...
    final headers = await _authHeaders();
    return await _withRefreshRetry(
      () => http.delete(uri, headers: headers, body: jsonEncode(dto.toJson())),
    );
  }
